...
static NSDateFormatter* CreateDateFormatter(NSString *format)
{
    ...
    [dateFormatter setLocale:AUTORELEASE([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"])];
    [dateFormatter setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"GMT"]];
    [dateFormatter setDateFormat:format];
    return AUTORELEASE(dateFormatter);
}
...
+ (NSDate *)dateFromHttpDateString:(NSString *)httpDate
{
    ...
    @synchronized(self) // NSDateFormatter isn't thread safe
    {
        ...
        if (!RFC1123DateFormatter) RFC1123DateFormatter = RETAIN(CreateDateFormatter(@"EEE, dd MMM yyyy HH:mm:ss z"));
        date = [RFC1123DateFormatter dateFromString:httpDate];
        if (!date)
        {
            // ANSI C date format - Sun Nov  6 08:49:37 1994
            if (!ANSICDateFormatter) ANSICDateFormatter = RETAIN(CreateDateFormatter(@"EEE MMM d HH:mm:ss yyyy"));
            date = [ANSICDateFormatter dateFromString:httpDate];
            if (!date)
            {
                // RFC 850 date format - Sunday, 06-Nov-94 08:49:37 GMT
                if (!RFC850DateFormatter) RFC850DateFormatter = RETAIN(CreateDateFormatter(@"EEEE, dd-MMM-yy HH:mm:ss z"));
...
+ (NSDate *)expirationDateFromHeaders:(NSDictionary *)headers withStatusCode:(NSInteger)status
{
    ...
    return AUTORELEASE([[NSDate alloc] initWithTimeInterval:kSDURLCacheDefault sinceDate:now]);
    ...
    if (!diskCacheInfo)
    {
        @synchronized(self)
        {
            if (!diskCacheInfo) // Check again, maybe another thread created it while waiting for the mutex
            {
                ...
                periodicMaintenanceTimer = RETAIN([NSTimer scheduledTimerWithTimeInterval:5
                                                                             target:self
...
- (void)periodicMaintenance
{
    ...
    if (diskCacheUsage > self.diskCapacity)
    {
        self.periodicMaintenanceOperation = AUTORELEASE([[NSInvocationOperation alloc] initWithTarget:self selector:@selector(balanceDiskUsage) object:nil]);
        [ioQueue addOperation:periodicMaintenanceOperation];
    }
    else if (diskCacheInfoDirty)
    {
        self.periodicMaintenanceOperation = AUTORELEASE([[NSInvocationOperation alloc] initWithTarget:self selector:@selector(saveCacheInfo) object:nil]);
        [ioQueue addOperation:periodicMaintenanceOperation];
    }
}
...
- (id)initWithMemoryCapacity:(NSUInteger)memoryCapacity diskCapacity:(NSUInteger)diskCapacity diskPath:(NSString *)path
{
    if ((self = [super initWithMemoryCapacity:memoryCapacity diskCapacity:diskCapacity diskPath:path]))
    {
        ...
        self.ioQueue = AUTORELEASE([[NSOperationQueue alloc] init]);
...
- (void)storeCachedResponse:(NSCachedURLResponse *)cachedResponse forRequest:(NSURLRequest *)request
{
        ...
        && cachedResponse.data.length < self.diskCapacity)
    {
        ...
        [ioQueue addOperation:AUTORELEASE(([[NSInvocationOperation alloc] initWithTarget:self
